...
import BusinessModel.Enterprise.VetHospital;
...
import BusinessModel.POJO.Pet;
import BusinessModel.UserAccount.UserAccount;
import BusinessModel.WorkQueue.PetAdoptionWorkRequest;
import BusinessModel.WorkQueue.PetHealthCheckUpRequest;
import BusinessModel.WorkQueue.ReportStrayAnimalRequest;
...
public class WardenDashBoardJPanel extends javax.swing.JPanel {
     ...
     */
    public WardenDashBoardJPanel(JPanel userProcessContainer, UserAccount account, PetShelterOrg organization, Enterprise enterprise, Environment business) {
        initComponents();
...
//                        : wq.getStatus().equalsIgnoreCase(filterStatus.getSelectedItem().toString()))
            ...
            case "Stray Animal Request":
                ...
                enterprise.getOrganizationDirectory().getOrganizationList().stream().filter((e) -> (e instanceof PetShelterOrg)).forEachOrdered((e) -> {
...
//                        : wq.getStatus().equalsIgnoreCase(filterStatus.getSelectedItem().toString()))
    ...
    private void reqApprovalBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_reqApprovalBtnActionPerformed
        ...
        PetAdoptionWorkRequest pa = (PetAdoptionWorkRequest) workRequestJTable.getValueAt(selectedRow, 0);
            ...
            pa.setReceiver(account);
            enterprise.getOrganizationDirectory().getOrganizationList().stream().filter(item -> item instanceof NgoOfficeOrg)
       ...
       if (selectedRow >= 0) {
           ...
           if (req.getStatus().equalsIgnoreCase("Warden Assigned")) {
                ...
                netwk.getEnterpriseDirectory().getEnterpriseList().stream().filter(item->item instanceof VetHospital)
                                                                            .forEach(item->item.getOrganizationDirectory().getOrganizationList().stream().filter(org->org instanceof ClinicOrg)
